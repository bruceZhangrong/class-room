...
export default {
  ...
  methods: {
    async loadInfo () {
      const json = await getScheduleById(this.$route.query.schedule_id)
      if (json.code === 1) {
    ...
    async back () {
      ...
      const json = await updateScheduleSeat(this.$route.query.schedule_id, JSON.stringify(this.seatInfo))
      if (json.code === 1) {

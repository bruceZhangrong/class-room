...
export default {
  ...
  methods: {
    async loadCinemaDetail () {
      if (this.$route.query.cinema_id) {
        ...
        if (json.code === 1) {
          ...
          const movieScheduleInfo = json.data.movieScheduleInfo
          const allMovieSchedule = []
    ...
    // 影片结束时间
    endDate (item) {
      const h = parseInt(Number(item.show_time.split(':')[0]) + (parseInt(item.movie_long) / 60))
      const m = Number(item.show_time.split(':')[1]) + parseInt(item.movie_long) % 60
      if (m > 59) {

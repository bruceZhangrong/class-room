...
const captcha = 'http://localhost:3000/api/captcha'
export default {
  name: 'Login',
  data () {
    return {
      phone: '18674454808', // 手机号码
      phoneCode: '', // 手机验证码
      userName: 'bruce', // 用户名
      password: '123456', // 密码
      captcha: '', // 图形验证码
  ...
  methods: {
    ...
    async getValidateCode () {
      if (this.checkPhone) {
        ...
        const json = await getPhoneCode(this.phone)
        if (json.code === 1) { // 获取手机验证码成功
    ...
    // 刷新图形验证码
    refreshCaptcha () {
      this.captchaSrc = `${captcha}?time=${new Date()}`
    },
    // 登录
    async login () {
      // 判断登录模式
      if (this.loginType) {
        ...
        if (this.phone === '') {
          ...
          const json = await phoneLogin(this.phone, this.phoneCode)
          if (json.code === 1) {
            Toast({
              message: '登录成功',
              position: 'middle',
        ...
        if (this.userName === '') {
          ...
          const json = await pwdLogin(this.userName, this.password, this.captcha)
          if (json.code === 1) {
            Toast({
              message: '登录成功',
              position: 'middle',
  ...
  .login-container {
    ...
    .login-content {
      ...
      .login-type {
        ...
        .active {
          color: #ffa500;
          border-bottom: 0.04rem solid #ffa500;
        }
      }

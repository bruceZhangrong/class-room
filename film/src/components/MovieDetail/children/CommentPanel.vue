...
export default {
  ...
  methods: {
    ...
    async loadUserComment () {
      const json = await getUserComment(this.$cookies.get('user_id'), this.$route.query.movie_id)
      if (json.code === 1) {
    ...
    async commentBtnHandle () {
      if (this.textarea) {
        ...
        const json = await updateUserComment(this.$cookies.get('user_id'), this.$route.query.movie_id, this.score, this.textarea, commentDate)
        if (json.code === 1) {

  ...
  methods: {
    ...
    async loadMovieDetail () {
      if (this.$route.query.movie_id) {
        const json = await getMovieDetail(this.$route.query.movie_id)
        if (json.code === 1) {
          ...
          if (this.$cookies.get('user_id')) {
            // 判断用户是否喜欢该电影
            const json = await isWishMovie(this.$cookies.get('user_id'), this.$route.query.movie_id)
            if (json.code === 1) {
          ...
          // 获取所有用户通过审核的评论
          const commentJson = await getAllUserPassComment(this.$route.query.movie_id)
          if (commentJson.code === 1 && commentJson.data.length) {
            let currentIndex = -1; let sum = 0
    ...
    async wishBtnHandle () {
      if (this.$cookies.get('user_id')) {
        ...
        if (this.notWishMovie) {
          const json = await wishMovie(this.$cookies.get('user_id'), this.$route.query.movie_id)
          if (json.code === 1) {
            this.notWishMovie = false
            this.loadMovieDetail()
          }
        } else {
          const json = await cancelWishMovie(this.$cookies.get('user_id'), this.$route.query.movie_id)
          if (json.code === 1) {
            this.notWishMovie = true
            this.loadMovieDetail()
          }

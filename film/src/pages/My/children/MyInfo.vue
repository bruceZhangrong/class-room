...
<script>
import Vue from 'vue'
import { MessageBox, Indicator, Toast, Message } from 'mint-ui'
  ...
  methods: {
    ...
    async modifyUserSex (sex) {
      if (this.$cookies.get('user_id') && sex) {
        const json = await updateUserSex(this.$cookies.get('user_id'), sex)
    ...
    async modifyUserAvatar () {
      if (!this.$cookies.get('user_id')) {
        ...
        if (json.code === 1) {
          if (json.data.length) {
            const userNewAvatar = '/images/avatar/' + json.data[0].filename
    ...
    async loadUserInfo () {
      if (this.$cookies.get('user_id')) {
        const json = await getUserInfo(this.$cookies.get('user_id'))
        if (json.code === 1) {
    ...
    async handleConfirm (selectedDate) {
      if (this.$cookies.get('user_id') && selectedDate) {
        const json = await updateUserBirthday(
          this.$cookies.get('user_id'),
          selectedDate
        )
        if (json.code === 1) {

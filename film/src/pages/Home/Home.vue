<template>
    <div id="home">
        <div class="search-header" :style="`background: rgba(255, 165, 0, ${headerBgOpcity})`">
          <span class="location" @click="changeAddress">广州</span>
          <span class="search"><span class="icon-search"></span><input type="text" placeholder="找电影、影院" @focus="$router.push('search_all')"></span>
          <span class="date"><span class="calender"><span class="day">{{currencyDate}}</span></span></span>
        </div>
...
export default {
  ...
  data () {
    ...
    return {
      ...
      hotMovieList: [],
      notShowMovieList: [],
      currencyDate: `${curDate < 10 ? '0' : ''}${curDate}`,
      headerBgOpcity: 0
    }
  },
  created () {
  ...
  methods: {
    ...
    // 处理滚动
    handleScroll () {
      const headHeight = document.querySelector('.swiper-container').clientHeight
      this.headerBgOpcity = window.pageYOffset > headHeight ? 1 : (window.pageYOffset / headHeight).toFixed(2)
    },
    // 加载电影列表
    async loadMovieList () {
      const json = await getMovieList()
      json.data.forEach((value, index) => {
        if (moment().isBefore(moment(value.public_date, 'YYYY-MM-DD'))) {
          this.hotMovieList.push(value)
        } else {
          this.notShowMovieList.push(value)
        }
      })
      this.hotMovieList.sort((a, b) => b.score - a.score)
